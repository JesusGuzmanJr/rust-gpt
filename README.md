...
A Rust implementation for training GPT models from scratch, including data preprocessing and tokenization.
...
cargo run --release --bin preprocess \
  --input-file /path/to/wikipedia.parquet \
  --output-dir /path/to/output

# Or using the justfile
just run preprocess -i /path/to/input.parquet -o /path/to/output
```
...
Parquet File → Producer (Rayon task)
         ...
         Consumer (par_bridge)
                 ↓
    Thread-local Encoders → Shard Files
```

The producer reads and processes batches in parallel, sends assembled articles through a channel, and consumers (Rayon workers) write to thread-local shard files in parallel.

## Tokenization

*(Documentation for tokenization coming soon)*

## Training

*(Documentation for training coming soon)*
